...
//    std::vector< XeInterlace<T> > deinter;
      ...
      // the Camera::Information class and __FITS_file class objects.
...
//        Camera::FitsKeys userkeys;    //!< create a FitsKeys object for FITS keys specified by the user
          __FITS_file *pFits;           //!< FITS container object has to be a pointer here
...
//    std::vector<arc::gen3::CArcDevice*> controller; //!< vector of arc::CController object pointers, one for each PCI device
      ...
      __FITS_file fits_file;            //!< instantiate a FITS container object

...
namespace Archon {
    ...
    long Interface::configure_controller() {
        ...
        for (int entry = 0; entry < this->config.n_entries; entry++) {
            ...
            if (config.param[entry].compare(0, 20, "TRIGIN_EXPOSE_ENABLE") == 0) {
            ...
            if (config.param[entry].compare(0, 21, "TRIGIN_EXPOSE_DISABLE") == 0) {
                // TRIGIN_EXPOSE_DISABLE
                int disable;
                try {
            ...
            if (config.param[entry].compare(0, 21, "TRIGIN_UNTIMED_ENABLE") == 0) {
            ...
            if (config.param[entry].compare(0, 22, "TRIGIN_UNTIMED_DISABLE") == 0) {
                // TRIGIN_UNTIMED_DISABLE
                int disable;
                try {
                    disable = std::stoi(config.arg[entry]);
                } catch (std::invalid_argument &) {
            ...
            if (config.param[entry].compare(0, 21, "TRIGIN_READOUT_ENABLE") == 0) {
            ...
            if (config.param[entry].compare(0, 22, "TRIGIN_READOUT_DISABLE") == 0) {
                // TRIGIN_READOUT_DISABLE
                int disable;
                try {
                    disable = std::stoi(config.arg[entry]);
                } catch (std::invalid_argument &) {
            ...
            if (config.param[entry].compare(0, 17, "SHUTENABLE_ENABLE") == 0) {
                // SHUTENABLE_ENABLE
                int enable;
                try {
                    enable = std::stoi(config.arg[entry]);
            ...
            if (config.param[entry].compare(0, 18, "SHUTENABLE_DISABLE") == 0) {
                // SHUTENABLE_DISABLE
                int disable;
                try {
                    disable = std::stoi(config.arg[entry]);
    ...
    long Interface::prepare_image_buffer() {
        ...
        if ((this->image_data != nullptr) &&
            ...
            message.str("");
            message << "initialized " << this->image_data_bytes << " bytes of image_data memory";
            logwrite(function, message.str());
        } else {
            ...
            if (this->image_data_bytes != 0) {
                ...
                this->image_data_allocated = this->image_data_bytes;
                message.str("");
                message << "allocated " << this->image_data_bytes << " bytes for image_data";
                logwrite(function, message.str());
            } else {
    ...
    long Interface::connect_controller(const std::string &devices_in = "") {
        ...
        if (this->archon.Connect() != 0) {
            message.str("");
            message << "connecting to " << this->camera_info.hostname << ":" << this->camera_info.port << ": " <<
                    strerror(errno);
        ...
        for (const auto &line: lines) {
            ...
            if ((tokens[0].compare(0, 3, "MOD") == 0) && (tokens[1] == "TYPE")) {
                try {
                    ...
                    message.str("");
                    message << "unable to convert module or type from " << tokens[0] << "=" << tokens[1] <<
                            " to integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "module " << tokens[0].substr(3) << " or type " << tokens[1] << " out of range";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            } else continue;
            ...
            //
            if (tokens[1] == "VERSION") version = tokens[2];
            else version = "";
            ...
            if ((module > 0) && (module <= nmods)) {
                try {
                    ...
                    message.str("");
                    message << "requested module " << module << " out of range {1:" << nmods;
                    this->camera.log_error(function, message.str());
                }
            } else {
                // else should never happen
                message.str("");
                message << "module " << module << " outside range {1:" << nmods << "}";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if ((type == 2 || type == 17) && (module < 5 || module > 8)) {
                message.str("");
                message << "AD module (type=" << type << ") cannot be in slot " << module << ". Use slots 5-8";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
     ...
     *
     */
    long Interface::archon_cmd(const std::string &cmd) {
        // use this form when the calling
    ...
    long Interface::archon_cmd(const std::string &cmd, std::string &reply) {
        ...
        try {
            ...
            message.str("");
            message << "converting Archon command: " << prefix << " to uppercase";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if (reply.compare(0, 1, "?") == 0) {
            ...
            this->camera.log_error(function, message.str());
        } else if (reply.compare(0, 3, check) != 0) {
            ...
            message.str("");
            message << "command-reply mismatch for command: " + scmd + ": expected " + check + " but received " + reply;
            this->camera.log_error(function, message.str());
        } else {
            ...
            if ((cmd.compare(0, 7, "WCONFIG") != 0) &&
                ...
                (cmd.compare(0, 5, "FRAME") != 0)) {
                message.str("");
                message << "command 0x" << std::setfill('0') << std::setw(2) << std::uppercase << std::hex << this->
                        msgref << " success";
                logwrite(function, message.str());
            }
    ...
    long Interface::read_parameter(const std::string &paramname, std::string &value) {
        ...
        if (this->parammap.find(paramname) == this->parammap.end()) {
            message.str("");
            message << "parameter \"" << paramname << "\" not found in ACF";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if (error != NO_ERROR) {
            message.str("");
            message << "ERROR: sending archon_cmd(" << cmd.str() << ")";
            logwrite(function, message.str());
            return error;
        }
        ...
        size_t loc;
        value = reply;
        if (value.compare(0, 9, "PARAMETER") == 0) {
            // value: PARAMETERn=PARAMNAME=VALUE
        ...
        if (error == ERROR) {
            message << "malformed reply: " << reply << " to Archon command " << cmd.str() <<
                    ": Expected PARAMETERn=PARAMNAME=VALUE";
            this->camera.log_error(function, message.str());
        } else {
            message.str("");
            message << paramname << " = " << value;
            logwrite(function, message.str());
        }
        return error;
    }
    ...
    long Interface::fetchlog() {
        ...
        //
        do {
            if ((retval = this->archon_cmd(FETCHLOG, reply)) != NO_ERROR) {
                // send command here
    ...
    long Interface::load_acf(std::string acffile) {
        ...
        while (getline(filestream, line)) {
            // note that getline discards the newline "\n" character
            ...
            //
            if (line.substr(0, 6) == "[MODE_") {
                // this is a mode section
                try {
                    line.erase(line.find('['), 1); // erase the opening square bracket
                    line.erase(line.find(']'), 1); // erase the closing square bracket
                } catch (...) {
                    message.str("");
                    message << "malformed mode section: " << savedline << ": expected [MODE_xxxx]";
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                if (!line.empty()) {
                    // What's remaining should be MODE_xxxxx
                    ...
                    if (this->modemap.find(mode) != this->modemap.end()) {
                        message.str("");
                        message << "duplicate definition of mode: " << mode << ": load aborted";
                        this->camera.log_error(function, message.str());
                        filestream.close();
                        return ERROR;
                    } else {
                    ...
                    // somehow there's no xxx left after removing "[MODE_" and "]"
                    message.str("");
                    message << "malformed mode section: " << savedline << ": expected [MODE_xxxx]";
                    this->camera.log_error(function, message.str());
                    filestream.close();
            ...
            if (line.compare(0, 4, "ACF:") == 0) {
                ...
                try {
                    ...
                    if (tokens.size() == 1) {
                        ...
                        message.str("");
                        message << "malformed ACF line: " << savedline << ": expected KEY=VALUE";
                        this->camera.log_error(function, message.str());
                        filestream.close();
                        return ERROR;
                    }
                    ...
                    if (!keymatch) {
                        message.str("");
                        message << "[MODE_" << mode << "] ACF directive: " << acf_key << "=" << acf_value <<
                                " is not a valid parameter or configuration key";
                        logwrite(function, message.str());
                        filestream.close();
                        return ERROR;
                    }
                    ...
                    message.str("");
                    message << "extracting KEY=VALUE pair from ACF line: " << savedline;
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                ...
                if (tokens.size() != 2) {
                    message.str("");
                    message << "malformed ARCH line: " << savedline << ": expected ARCH:KEY=VALUE";
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                if (tokens[0] == "NUM_DETECT") {
                    ...
                    message.str("");
                    message << "unrecognized internal parameter specified: " << tokens[0];
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                ...
                if (tokens.size() != 2) {
                    // need at least two tokens at this point
                    message.str("");
                    message << "malformed FITS command: " << savedline << ": expected KEYWORD=value/comment";
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                ...
                if (tokens.empty()) {
                    // no tokens found means no "/" characeter which means no comment
                ...
                if (tokens.size() == 2) {
                    // If there are two tokens here then the second is a comment
                    keycomment = tokens[1];
                }

                if (tokens.size() > 2) {
                    // everything below this has been covered
                    message.str("");
                    message << "malformed FITS command: " << savedline << ": expected KEYWORD=VALUE/COMMENT";
                    this->camera.log_error(function, message.str());
                    message.str("");
                    message << "too many \"/\" in comment string? " << keystring;
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                ...
                //
                       ...
                       (line.compare(0, 9, "PARAMETER") == 0)) {
                // but must start with "PARAMETER"
                ...
                if (tokens.size() != 3) {
                    message.str("");
                    message << "malformed paramter line: " << savedline << ": expected PARAMETERn=Param=value";
                    this->camera.log_error(function, message.str());
                    filestream.close();
                    return ERROR;
                }
                ...
                if (tokens.size() > 1) {
                    // if a second token then that's the value
    ...
    long Interface::set_camera_mode(std::string mode) {
        ...
        if (this->modemap.find(mode) == this->modemap.end()) {
            message.str("");
            message << "undefined mode " << mode << " in ACF file " << this->camera.firmware[0];
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if (load_mode_settings(mode) != NO_ERROR) {
            message.str("");
            message << "ERROR: failed to load mode settings for mode: " << mode;
            logwrite(function, message.str());
            return ERROR;
        }
    ...
    logwrite(function, message.str());
        ...
        //
        if (error != NO_ERROR) {
            logwrite(function, "ERROR: one or more internal variables missing from configmap");
            return error;
        }
        ...
        //
        int bigbuf = -1;
        if (error == NO_ERROR) error = get_configmap_value("BIGBUF", bigbuf);
        // get value of BIGBUF from loaded acf file
        this->camera_info.activebufs = (bigbuf == 1) ? 2 : 3; // set number of active buffers based on BIGBUF
        if (error != NO_ERROR) {
            logwrite(function, "ERROR: unable to read BIGBUF from ACF");
            return error;
        }
      ...
      logwrite(function, message.str());
#endif
            if (error != NO_ERROR) {
                logwrite(function, "ERROR: unable to get PIXELCOUNT,LINECOUNT from ACF");
                return error;
            }
        }
        ...
        // SAMPLEMODE=0 for 16bpp, =1 for 32bpp
        if (error != NO_ERROR) {
            logwrite(function, "ERROR: unable to get SAMPLEMODE from ACF");
            return error;
        }
        if (samplemode < 0) {
            this->camera.log_error(function, "bad or missing SAMPLEMODE from ACF");
            return ERROR;
        }
        ...
        // TODO I think paramchanged is never set!
        if ((error == NO_ERROR) && configchanged) error = this->archon_cmd(APPLYCDS);
        // TODO I think configchanged is never set!
        ...
        *********/
        ...
        if (this->image_data_bytes == 0) {
            this->camera.log_error(
                function, "image data size is zero! check NUM_DETECT, HORI_AMPS, VERT_AMPS in .acf file");
            error = ERROR;
        }

        this->camera_info.current_observing_mode = mode;
        // identify the newly selected mode in the camera_info class object
        this->modeselected = true; // a valid mode has been selected

        message.str("");
        message << "new mode: " << mode << " will use " << this->camera_info.bitpix << " bits per pixel";
        logwrite(function, message.str());
        ...
        message.str("");
        message << "identified " << this->camera_info.amp_section.size() << " amplifier sections";
        logwrite(function, message.str());
    ...
    long Interface::load_mode_settings(std::string mode) {
        ...
        if (!this->shutenableparam.empty()) {
            ...
            if (read_parameter(this->shutenableparam, lshutten) != NO_ERROR) {
                message.str("");
                message << "ERROR: reading \"" << this->shutenableparam << "\" parameter from Archon";
                logwrite(function, message.str());
                return ERROR;
            }
            ...
            if (lshutten == "1") shuttenstr = "enable";
                ...
                message.str("");
                message << "ERROR: unrecognized shutter enable parameter value " << lshutten << ": expected {0,1}";
                logwrite(function, message.str());
                return ERROR;
            }
            ...
            //
            std::string dontcare;
            if (this->shutter(shuttenstr, dontcare) != NO_ERROR) {
                logwrite(function, "ERROR: setting shutter enable parameter");
                return ERROR;
            }
        }
        ...
        for (int tapn = 0; tapn < this->taplines; tapn++) {
            ...
            if (tokens.size() == 3) {
                ...
                if (adchan.find("AD") != std::string::npos) admax = MAXADCCHANS;
                    ...
                    message.str("");
                    message << "bad tapline syntax. Expected ADn or AMn but got " << adchan;
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ...
                try {
                    ...
                    message.str("");
                    message << "unable to convert AD number \'" << adchan << "\' to integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                ...
                if ((adnum < 0) || (adnum > admax)) {
                    message.str("");
                    message << "ADC channel " << adnum << " outside range {0:" << admax << "}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ...
                try {
                    ...
                    message.str("");
                    message << "unable to convert GAIN \"" << tokens[1] << "\" and/or OFFSET \"" << tokens[2] <<
                            "\" to integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "GAIN " << tokens[1] << ", OFFSET " << tokens[2] << " outside integer range";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ...
                try {
                    ...
                    std::stringstream keystr;
                    keystr.str("");
                    keystr << "GAIN" << std::setfill('0') << std::setw(2) << adnum << "=" << this->gain.at(adnum) <<
                            "// gain for AD chan " << adnum;
                    this->systemkeys.addkey(keystr.str());
                    keystr.str("");
                    keystr << "OFFSET" << std::setfill('0') << std::setw(2) << adnum << "=" << this->offset.at(adnum) <<
                            "// offset for AD chan " << adnum;
                    this->systemkeys.addkey(keystr.str());
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "AD# " << adnum << " outside range {0:" << (this->gain.size() & this->offset.size()) <<
                            "}";
                    this->camera.log_error(function, message.str());
                    if (this->gain.empty() || this->offset.empty()) {
                        this->camera.log_error(
                            function, "gain/offset vectors are empty: no ADC or ADM board installed?");
                    }
                    return ERROR;
                }
#ifdef LOGLEVEL_DEBUG
    ...
    long Interface::get_frame_status() {
        ...
        for (const auto &token: tokens) {
            ...
            if (subtokens.size() != 2) {
                message.str("");
                message << "expected 2 but received invalid number of tokens (" << subtokens.size() <<
                        ") in FRAME message:";
            ...
            if (subtokens[0] == "TIMER") {
                ...
                try {
                    ...
                    message.str("");
                    message << "unable to convert buffer: " << subtokens[0] << " or value: " << subtokens[1] <<
                            " from FRAME message to integer. Expected BUFnSOMETHING=nnnn";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "buffer: " << subtokens[0] << " or value: " << subtokens[1] <<
                            " from FRAME message outside integer range. Expected BUFnSOMETHING=nnnn";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            ...
            if (subtokens[0].compare(0, 3, "BUF") == 0) {
                ...
                if (bufnum < 1 || bufnum > Archon::nbufs) {
                    message.str("");
                    message << "buffer number " << bufnum << " from FRAME message outside range {1:" << Archon::nbufs <<
                            "}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
        ...
        if (this->frame.index < (int) this->frame.bufframen.size()) {
            ...
            message.str("");
            message << "newest buf " << this->frame.index << " from FRAME message exceeds number of buffers " << this->
                    frame.bufframen.size();
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::lock_buffer(int buffer) {
        ...
        if (this->archon_cmd(sscmd.str())) {
            message.str("");
            message << "ERROR: sending Archon command to lock frame buffer " << buffer;
            logwrite(function, message.str());
            return ERROR;
        }
        return (NO_ERROR);
    ...
    long Interface::get_timer(unsigned long int *timer) {
        ...
        if (tokens.size() != 2) {
            message.str("");
            message << "unrecognized timer response: " << reply << ". Expected TIMER=xxxx";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::fetch(uint64_t bufaddr, uint32_t bufblocks) {
        ...
        if (bufaddr > maxaddr) {
            message.str("");
            message << "fetch Archon buffer requested address 0x" << std::hex << bufaddr << " exceeds 0x" << maxaddr;
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        if (bufblocks > maxblocks) {
            message.str("");
            message << "fetch Archon buffer requested blocks 0x" << std::hex << bufblocks << " exceeds 0x" << maxblocks;
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        try {
            ...
            message.str("");
            message << "converting command: " << sscmd.str() << " to uppercase";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        message.str("");
        message << "reading " << (this->camera_info.frame_type == Camera::FRAME_RAW ? "raw" : "image") << " with " <<
                scmd;
        logwrite(function, message.str());
        return NO_ERROR;
    }
    ...
    long Interface::read_frame() {
        ...
        // RAW-only
        //
        if (this->camera_info.current_observing_mode == "RAW") {
            // "RAW" is the only reserved mode name

            // the RAWENABLE parameter must be set in the ACF file, in order to read RAW data
            //
            if (rawenable == 0) {
                this->camera.log_error(
                    function, "observing mode is RAW but RAWENABLE=0 -- change mode or set RAWENABLE?");
                return ERROR;
            } else {
                ...
                if (error != NO_ERROR) {
                    logwrite(function, "ERROR: writing raw frame");
                    return error;
                }
            }
        } else {
            ...
            if (error != NO_ERROR) {
                logwrite(function, "ERROR: reading image frame");
                return error;
            }
            error = this->write_frame(); // write image frame
            if (error != NO_ERROR) {
                logwrite(function, "ERROR: writing image frame");
                return error;
            }
...
#endif
                std::string orig_mode = this->camera_info.current_observing_mode;
                // save the original mode, so we can come back to it
                error = this->set_camera_mode("raw"); // switch to raw mode
                if (error != NO_ERROR) {
                    logwrite(function, "ERROR: switching to raw mode");
                    return error;
                }
        ...
        message.str(""); message << "error=" << error << "[DEBUG] switching back to original mode if error=0"; logwrite(function, message.str());
                ...
                if (error != NO_ERROR) {
                    logwrite(function, "ERROR: switching back to previous mode");
                    return error;
                }
            }
        }
    ...
    long Interface::hread_frame() {
        ...
        if (bufready < 1 || bufready > this->camera_info.activebufs) {
            message.str("");
            message << "invalid Archon buffer " << bufready << " requested. Expected {1:" << this->camera_info.
                    activebufs << "}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        message.str("");
        message << "will read image data from Archon controller buffer " << bufready << " frame " << this->frame.frame;
        logwrite(function, message.str());
        ...
        message.str("");
        message << "will read " << std::dec << this->camera_info.image_memory << " bytes "
        ...
        for (block = 0; block < bufblocks; block++) {
            ...
            while (this->archon.Bytes_ready() < (BLOCK_LEN + 4)) {
                ...
                if (diff.count() > 1) {
                    // break while loop if duration > 1 second
                    std::cerr << "\n";
            ...
            if ((retval = this->archon.Read(header, 4)) != 4) {
                message.str("");
                message << "code " << retval << " reading Archon frame header";
                this->camera.log_error(function, message.str());
                error = ERROR;
                break; // break out of for loop
            }
            if (header[0] == '?') {
                // Archon retured an error
                message.str("");
                message << "Archon returned \'?\' reading image data";
                this->camera.log_error(function, message.str());
                ...
                message.str("");
                message << "Archon command-reply mismatch reading image data. header=" << header << " check=" << check;
                this->camera.log_error(function, message.str());
                error = ERROR;
                break; // break out of for loop
            }
        ...
        if (error == ERROR || block < bufblocks) {
            message.str("");
            message << "incomplete frame read " << std::dec
        ...
        if (error == NO_ERROR) {
            message.str("");
            message << "successfully read " << std::dec << totalbytesread
                    << " image bytes (0x" << std::uppercase << std::hex << bufblocks <<
                    " blocks) from Archon controller";
            logwrite(function, message.str());
        } else {
            // Throw an error for any other errors
    ...
    long Interface::read_frame(Camera::frame_type_t frame_type) {
        ...
        ***/
        ...
        message.str("");
        message << "will read " << (frame_type == Camera::FRAME_RAW ? "raw" : "image")
        ...
        switch (frame_type) {
            ...
            default: // should be impossible
                message.str("");
                message << "unknown frame type specified: " << frame_type << ": expected FRAME_RAW | FRAME_IMAGE";
                this->camera.log_error(function, message.str());
                return ERROR;
                break;
        }
        ...
        for (block = 0; block < bufblocks; block++) {
            ...
            if (header[0] == '?') {
                // Archon retured an error
                message.str("");
                message << "Archon returned \'?\' reading " << (frame_type == Camera::FRAME_RAW ? "raw " : "image ") <<
                        " data";
                this->camera.log_error(function, message.str());
                this->fetchlog(); // check the Archon log for error messages
                error = ERROR;
                ...
                message.str("");
                message << "Archon command-reply mismatch reading " << (frame_type == Camera::FRAME_RAW
                                                                            ? "raw "
                                                                            : "image ")
        ...
        if (error == NO_ERROR) {
            message.str("");
            message << "successfully read " << std::dec << totalbytesread << (frame_type == Camera::FRAME_RAW
                                                                                  ? " raw"
                                                                                  : " image")
    ...
    long Interface::write_frame() {
        ...
        message.str("");
        message << "writing " << this->camera_info.bitpix << "-bit data from memory to disk";
        logwrite(function, message.str());
        ...
        switch (this->camera_info.bitpix) {
            ...
            case 32: {
              ...
              logwrite( function, message.str() );
                            ...
                            if (fext != nullptr) {
                                delete [] fext;
                                fext = nullptr;
                            } // dynamic object not automatic so must be destroyed
                        } catch (std::out_of_range &) {
                            message.str("");
                            message << "ERROR: " << ext << " is a bad extension number";
                            logwrite(function, message.str());
                            if (fext != nullptr) {
                                delete [] fext;
                                fext = nullptr;
                    ...
                    // allocate a float buffer of same number of pixels for scaling

                    //          for (long pix=0; pix < this->fits_info.section_size; pix++)   //TODO
                    for (long pix = 0; pix < this->camera_info.section_size; pix++) {
                        fbuf[pix] = (float) (cbuf32[pix] >> this->n_hdrshift);
                        // right shift the requested number of bits
                    }
            ...
            //
            case 16: {
                if (this->camera_info.datatype == USHORT_IMG) {
                    // raw
                    ...
                    message.str("");
                    message << "unsupported 16 bit datatype " << this->camera_info.datatype;
                    this->camera.log_error(function, message.str());
                    error = ERROR;
                }
                break;
            ...
            default:
                ...
                message.str("");
                message << "unrecognized bits per pixel: " << this->camera_info.bitpix;
                this->camera.log_error(function, message.str());
                error = ERROR;
                break;
        }
        ...
        if (error == NO_ERROR) {
            if (this->camera.datacube()) {
                this->camera_info.extension++; // increment extension for cubes
                message.str("");
                message << "DATACUBE:" << this->camera_info.extension << " " << (error == NO_ERROR
                    ? "COMPLETE"
                    : "ERROR");
                this->camera.async.enqueue(message.str());
                error == NO_ERROR ? logwrite(function, message.str()) : this->camera.log_error(function, message.str());
    ...
    long Interface::write_config_key(const char *key, const char *newvalue, bool &changed) {
        ...
        if (key == nullptr || newvalue == nullptr) {
            ...
            error = ERROR;
            message.str("");
            message << "requested key " << key << " not found in configmap";
            this->camera.log_error(function, message.str());
        } else if (this->configmap[key].value == newvalue) {
            // If no change in value then don't send the command
            error = NO_ERROR;
            message.str("");
            message << "config key " << key << "=" << newvalue << " not written: no change in value";
            logwrite(function, message.str());
        } else {
            /**
         ...
         */
            sscmd << "WCONFIG"
                    ...
                    << newvalue;
            message.str("");
            message << "sending: archon_cmd(" << sscmd.str() << ")";
            logwrite(function, message.str());
            ...
            if (error == NO_ERROR) {
                ...
                message.str("");
                message << "ERROR: config key=value: " << key << "=" << newvalue << " not written";
                logwrite(function, message.str());
            }
        }
        return error;
    ...
    logwrite( function, message.str() );
        ...
        if (paramname == nullptr || newvalue == nullptr) {
            ...
            error = ERROR;
            message.str("");
            message << "parameter \"" << paramname << "\" not found in parammap";
            this->camera.log_error(function, message.str());
        }
        ...
        if (error == NO_ERROR && this->parammap[paramname].value == newvalue) {
            error = NO_ERROR;
            message.str("");
            message << "parameter " << paramname << "=" << newvalue << " not written: no change in value";
            logwrite(function, message.str());
    ...
    long Interface::expose(std::string nseq_in) {
        ...
        if (!this->camera.cubeamps() && (this->lastcubeamps != this->camera.cubeamps())) {
            message.str("");
            message << "detected change in cubeamps -- resetting camera mode to " << mode;
            logwrite(function, message.str());
        ...
        if (this->camera_info.exposure_time == -1) {
            ...
            //
            std::string retval;
            if (this->exptime(etime, retval) != NO_ERROR) {
                logwrite(function, "ERROR: setting exptime");
                return ERROR;
            }
        }
        if (this->camera_info.exposure_factor == -1 ||
            ...
            //
            std::string lexp;
            if (read_parameter("longexposure", lexp) != NO_ERROR) {
        ...
        if (nseq_in.empty()) {
            ...
            try {
                ...
                message.str("");
                message << "unable to convert sequences: " << nseq_in << " to integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "sequences " << nseq_in << " outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
        ...
        // copy the systemkeys database object into camera_info

        if (this->camera.writekeys_when == "before") this->copy_keydb();
        // copy the ACF and userkeys database into camera_info
        ...
        if ((mode != "RAW") && (this->modemap[mode].rawenable)) {
            if (!this->camera.datacube()) {
                // if datacube not already set then it must be overridden here
      ...
      logwrite( function, "[DEBUG] opening fits file for multi-exposure sequence data cube" );
        ...
        if (nseq > 1) {
            message.str("");
            message << "starting sequence of " << nseq << " frames. lastframe=" << this->lastframe;
            logwrite(function, message.str());
        }
        ...
        if (mode != "RAW") {
            ...
            while (nseq-- > 0) {
                ...
                if (++expcount <= this->camera_info.num_pre_exposures) {
                    message.str("");
                    message << "pre-exposure " << expcount << " of " << this->camera_info.num_pre_exposures;
                    logwrite(function, message.str());
        ...
        logwrite( function, message.str() );
                ...
                if (!this->camera.datacube() || this->camera.cubeamps()) {
                    ...
                    // current system time formatted as YYYY-MM-DDTHH:MM:SS.sss
                    if (this->get_timer(&this->start_timer) != NO_ERROR) {
                        // Archon internal timer (one tick=10 nsec)
                        logwrite(function, "ERROR: could not get start time");
                        return ERROR;
          ...
          logwrite( function, "[DEBUG] reset extension=0 and opening new fits file" );
                ...
                if (this->camera.writekeys_when == "after") this->copy_keydb();
                // copy the ACF and userkeys database into camera_info
          ...
          logwrite( function, "[DEBUG] closing fits file (1)" );
#endif
                    this->fits_file.close_file(this->camera.writekeys_when == "after", this->camera_info);
                    // close the file when not using datacubes
                    this->camera.increment_imnum(); // increment image_num when fitsnaming == "number"

                    // ASYNC status message on completion of each file
                    //
                    message.str("");
                    message << "FILE:" << this->camera_info.fits_name << " COMPLETE";
                    this->camera.async.enqueue(message.str());
                    logwrite(function, message.str());
                }

                if (error != NO_ERROR) break;
                // should be impossible but don't try additional sequences if there were errors
            ...
            this->camera_info.systemkeys.keydb = this->systemkeys.keydb;
            // copy the systemkeys database into camera_info
      ...
      logwrite( function, "[DEBUG] closing fits file (2)" );
            ...
            //
            message.str("");
            message << "FILE:" << this->camera_info.fits_name << " " << (error == NO_ERROR ? "COMPLETE" : "ERROR");
            this->camera.async.enqueue(message.str());
            error == NO_ERROR ? logwrite(function, message.str()) : this->camera.log_error(function, message.str());
        }
    ...
    long Interface::hroi(std::string geom_in, std::string &retstring) {
        ...
        //
        if (!geom_in.empty()) {
            // geometry arguments passed in
            Tokenize(geom_in, tokens, " ");

            if (tokens.size() != 4) {
                message.str("");
                message << "param expected 4 arguments (vstart, vstop, hstart, hstop) but got " << tokens.size();
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            try {
                ...
                message.str("");
                message << "unable to convert geometry values: " << geom_in << " to integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "geometry values " << geom_in << " outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if (vstart < 0 || vstop > 2047 || hstart < 0 || hstop > 2047) {
                message.str("");
                message << "geometry values " << geom_in << " outside pixel range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if (vstart >= vstop || hstart >= hstop) {
                message.str("");
                message << "geometry values " << geom_in << " are not correctly ordered";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        ...
        //
        message.str("");
        message << this->win_vstart << " " << this->win_vstop << " " << this->win_hstart << " " << this->win_hstop;
        retstring = message.str();

        if (error != NO_ERROR) {
            message.str("");
            message << "setting window geometry to " << retstring;
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::hwindow(std::string state_in, std::string &state_out) {
        ...
        if (!state_in.empty()) {
            try {
                ...
                if (state_in == "FALSE" || state_in == "0") {
                    ...
                    message.str("");
                    message << "window state " << state_in << " is invalid. Expecting {true,false,0,1}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            } catch (...) {
                message.str("");
                message << "unknown exception converting window state " << state_in << " to uppercase";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
        ...
        if (error != NO_ERROR) {
            message.str("");
            message << "setting window state to " << state_in;
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::autofetch(std::string state_in, std::string &state_out) {
        ...
        if (!state_in.empty()) {
            try {
                ...
                if (state_in == "FALSE" || state_in == "0") {
                    ...
                    message.str("");
                    message << "autofetch state " << state_in << " is invalid. Expecting {true,false,0,1}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            } catch (...) {
                message.str("");
                message << "unknown exception converting autofetch state " << state_in << " to uppercase";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
        ...
        if (error != NO_ERROR) {
            message.str("");
            message << "setting autofetch state to " << state_in;
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::hexpose(std::string nseq_in) {
        ...
        if (nseq > 1) {
            message.str("");
            message << "starting sequence of " << nseq << " frames. lastframe=" << this->lastframe << " last buffer=" <<
                    currentindex + 1;
            logwrite(function, message.str());
        }
        ...
        message.str("");
        message << "READOUT SEQUENCE " << (error == NO_ERROR ? "COMPLETE" : "ERROR") << " (" << nread << " of " << nseq
                << " read)";
        this->camera.async.enqueue(message.str());
        error == NO_ERROR ? logwrite(function, message.str()) : this->camera.log_error(function, message.str());
        ...
        message.str("");
        message << "Last frame read " << this->frame.frame << " from buffer " << this->frame.index + 1;
        logwrite(function, message.str());
...
#endif
                    this->fits_file.close_file(
                        this->camera.writekeys_when == "after",
                        this->camera_info); // close the file when not using datacubes
                    this->camera.increment_imnum(); // increment image_num when fitsnaming == "number"
    ...
    long Interface::wait_for_exposure() {
        ...
        if (this->is_longexposure) {
            ...
            waittime = floor(0.9 * this->camera_info.exposure_time / this->camera_info.exposure_factor);
            // in units of this->camera_info.exposure_unit
        }
        ...
        //
        unsigned long int prediction = this->start_timer + this->camera_info.exposure_time * 1e8 / this->camera_info.
                                       exposure_factor;
        ...
        while ((now - (waittime + start_time) < 0) && !this->abort) {
            ...
            if (this->camera_info.exposure_progress < 0 || this->camera_info.exposure_progress > 1)
                this->camera_info.exposure_progress = 1;
            ...
            //
            message.str("");
            message << "EXPOSURE:" << (int) (this->camera_info.exposure_time - (
                                                 this->camera_info.exposure_progress * this->camera_info.
                                                 exposure_time));
            this->camera.async.enqueue(message.str());
        }
        ...
        while (!done && !this->abort) {
            ...
            this->camera_info.exposure_progress =
                    (double) (timer - this->start_timer) / (double) (prediction - this->start_timer);
            if (this->camera_info.exposure_progress < 0 || this->camera_info.exposure_progress > 1)
                this->camera_info.exposure_progress = 1;
            ...
            std::cerr << std::setw(3) << (int) (this->camera_info.exposure_progress * 100) << "\b\b\b";
            // send to stderr in case anyone is watching
            ...
            //
            if ((timer - this->start_timer) >= (this->camera_info.exposure_time * 1e8 / this->camera_info.
                                                exposure_factor)) {
    ...
    long Interface::wait_for_readout() {
        ...
        try {
            ...
            message.str("");
            message << "readout time for Archon not found from config file";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        while (!done && !this->abort) {
            ...
            if (error == BUSY) {
                if (++busycount > 300) {
                    ...
                    this->camera.log_error(
                        function, "received BUSY from Archon too many times trying to get frame status");
                    break;
                } else continue;
            } else busycount = 0;
            ...
            if (clock_now > clock_timeout) {
                ...
                error = ERROR;
                message.str("");
                message << "timeout waiting for new frame exceeded " << waittime << ". lastframe = " << this->lastframe;
                this->camera.log_error(function, message.str());
                break;
            }
            ...
            //
            message.str("");
            message << "LINECOUNT:" << this->frame.buflines[this->frame.next_index];
#ifdef LOGLEVEL_DEBUG
      message << " [DEBUG] ";
      ...
      for ( int i=0; i < Archon::nbufs; i++ ) { message << " " << this->frame.buflines[ i ]; }
        ...
        if (error == NO_ERROR) {
            ...
            message.str("");
            message << "LINECOUNT:" << this->frame.buflines[this->frame.index];
            this->camera.async.enqueue(message.str());
        }
    ...
    long Interface::set_parameter(std::string parameter) {
        ...
        if (tokens.size() != 2) {
            message.str("");
            message << "param expected 2 arguments (paramname and value) but got " << tokens.size();
            this->camera.log_error(function, message.str());
            ret = ERROR;
        } else {
    ...
    long Interface::exptime(std::string exptime_in, std::string &retstring) {
        ...
        if (!exptime_in.empty()) {
            ...
            try {
                ...
                message.str("");
                message << "converting exposure time: " << exptime_in << " to integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "requested exposure time: " << exptime_in << " outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if (exp_time < 0 || exp_time > 0xFFFFF) {
                message.str("");
                message << "requested exposure time: " << exptime_in << " out of range {0:1048575}";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        ...
        //
        message.str("");
        message << "EXPTIME=" << this->camera_info.exposure_time << " // exposure time in " << (
            this->is_longexposure ? "sec" : "msec");
        this->systemkeys.addkey(message.str());

        // prepare the return value
        //
        message.str("");
        message << this->camera_info.exposure_time << (this->is_longexposure ? " sec" : " msec");
        retstring = message.str();

        message.str("");
        message << "exposure time is " << retstring;
        logwrite(function, message.str());
    ...
    long Interface::shutter(std::string shutter_in, std::string &shutter_out) {
        ...
        if (!shutter_in.empty()) {
            try {
                ...
                if (shutter_in == "disable") {
                    ...
                    message.str("");
                    message << shutter_in << " is invalid. Expecting { enable | disable | open | close | reset }";
                    this->camera.log_error(function, message.str());
                    error = ERROR;
                }
                ...
                if (error == NO_ERROR && ability) {
                    std::stringstream cmd;
                    cmd << this->shutenableparam << " " << (
                        shutten ? this->shutenable_enable : this->shutenable_disable);
                    error = this->set_parameter(cmd.str());
                ...
                message.str("");
                message << "converting requested shutter state: " << shutter_in << " to lowercase";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
        ...
        if (force) {
            message.str("");
            message << "SHUTFORC=" << level << "// shutter force level";
            this->systemkeys.addkey(message.str());
        }
        ...
        //
        message.str("");
        message << "SHUTTEN=" << (this->camera_info.shutterenable ? "T" : "F") << "// shutter was enabled";
        this->systemkeys.addkey(message.str());
    ...
    long Interface::hdrshift(std::string bits_in, std::string &bits_out) {
        ...
        if (!bits_in.empty()) {
            try {
                ...
                message.str("");
                message << "converting hdrshift: " << bits_in << " to integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "hdrshift: " << bits_in << " is outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
    ...
    long Interface::trigin(std::string state_in) {
        ...
        if (tokens.empty() || tokens.size() > 2) {
            message.str("");
            message << "requested trigin state: " << state_in <<
                    " is invalid. Expected { expose [N] | untimed | readout | disable }";
            this->camera.log_error(function, message.str());
            error = ERROR;
        }
        ...
        try {
            ...
            //
            if (tokens.at(0) == "expose") {
                // timed exposures can take an optional argument
                // At minimum, expose requires that TRIGIN_EXPOSE_PARAM be defined
                //
                if (this->trigin_exposeparam.empty()) {
                    message.str("");
                    message << "TRIGIN_EXPOSE_PARAM not defined in configuration file " << this->config.filename;
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                if (tokens.size() == 1) {
                    ...
                    // size given so try to set trigin_expose to requested value
            ...
            } else if (tokens.at(0) == "untimed") {
                // untimed exposures
                // At minimum, untimed requires that TRIGIN_UNTIMED_PARAM be defined
                if (this->trigin_untimedparam.empty()) {
                    message.str("");
                    message << "TRIGIN_UNTIMED_PARAM not defined in configuration file " << this->config.filename;
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                this->trigin_expose = 0;
                ...
                this->trigin_readout = this->trigin_readout_disable;
            } else if (tokens.at(0) == "readout") {
                // readout is used at the end of untimed exposures
                // At minimum, readout requires that TRIGIN_READOUT_PARAM be defined
                if (this->trigin_readoutparam.empty()) {
                    message.str("");
                    message << "TRIGIN_READOUT_PARAM not defined in configuration file " << this->config.filename;
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                this->trigin_expose = 0;
            ...
            } else if (tokens.at(0) == "disable") {
                // used to disable untimed exposures
                this->trigin_expose = 0;
                this->trigin_untimed = this->trigin_untimed_disable;
                this->trigin_readout = this->trigin_readout_disable;
            } else {
            ...
            message.str("");
            message << "converting number of timed external-trigger exposures: " << state_in << " to integer";
            this->camera.log_error(function, message.str());
            error = ERROR;
        } catch (std::out_of_range &) {
            message.str("");
            message << "parsing trigin string: " << state_in;
            this->camera.log_error(function, message.str());
            error = ERROR;
        } catch (...) {
            message.str("");
            message << "unknown exception parsing trigin string: " << state_in;
            this->camera.log_error(function, message.str());
            error = ERROR;
        }
    ...
    logwrite( function, message.str() );
        ...
        if (error == NO_ERROR && this->trigin_state == "disable") {
            if (this->fits_file.isopen()) {
                // "untimed" could have opened a FITS file
        ...
        if (this->trigin_state == "readout") {
            ...
            if (this->trigin_expose > 1) {
                message.str("");
                message << "starting sequence of " << this->trigin_expose << " frames. lastframe=" << this->lastframe;
                logwrite(function, message.str());
            }
            ...
            if ((error == NO_ERROR) && (mode != "RAW")) {
                ...
                while (this->trigin_expose-- > 0) {
                    ...
                    if (error == NO_ERROR && this->camera_info.exposure_time != 0) {
                        // wait for the exposure delay to complete (if there is one)
                        error = this->wait_for_exposure();
                    }
                    ...
                    if (!this->camera.datacube()) {
                        ...
                        //
                        message.str("");
                        message << "FILE:" << this->camera_info.fits_name << " " << (error == NO_ERROR
                            ? "COMPLETE"
                            : "ERROR");
                        this->camera.async.enqueue(message.str());
                        error == NO_ERROR
                ...
                error = this->get_frame_status(); // Get the current frame buffer status
                if (error == NO_ERROR) error = this->camera.get_fitsname(this->camera_info.fits_name);
            ...
            message.str("");
            message << "unexpected error processing trigin state " << this->trigin_state;
            this->camera.log_error(function, message.str());
            error = ERROR;
        }
    ...
    long Interface::longexposure(std::string state_in, std::string &state_out) {
        ...
        if (!state_in.empty()) {
            try {
                ...
                else if (state_in == "TRUE" || state_in == "1") this->is_longexposure = true;
                else {
                    message.str("");
                    message << "longexposure state " << state_in << " is invalid. Expecting {true,false,0,1}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            } catch (...) {
                message.str("");
                message << "unknown exception converting longexposure state " << state_in << " to uppercase";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
    ...
    long Interface::heater(std::string args, std::string &retstring) {
        ...
        try {
            ...
            if (heaterid != "A" && heaterid != "B") {
                message.str("");
                message << "invalid heater " << heaterid << ": expected <module> A|B";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        } catch (std::invalid_argument &) {
            message.str("");
            message << "converting heater <module> " << tokens[0] << " to integer";
            this->camera.log_error(function, message.str());
            return ERROR;
        } catch (std::out_of_range &) {
            message.str("");
            message << "heater <module>: " << tokens[0] << " is outside integer range";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        switch (this->modtype[module - 1]) {
            ...
            default:
                message.str("");
                message << "module " << module << " not a heater board";
                this->camera.log_error(function, message.str());
                return ERROR;
        }
        ...
        if (ret == -999) {
            message.str("");
            message << "comparing backplane version " << this->backplaneversion << " to " << REV_HEATERTARGET;
            this->camera.log_error(function, message.str());
            return ERROR;
        } else if (ret == -1) {
        ...
        //
        if (tokens.size() == 2) {
            // no args reads ENABLE, TARGET
            readonly = true;
            ss.str("");
            ss << "MOD" << module << "/HEATER" << heaterid << "ENABLE";
            heaterconfig.push_back(ss.str());
            ss.str("");
            ss << "MOD" << module << "/HEATER" << heaterid << "TARGET";
            heaterconfig.push_back(ss.str());
            ...
            if (tokens[2] == "ON") {
                ...
                readonly = true;
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "RAMP";
                heaterconfig.push_back(ss.str());
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "RAMPRATE";
                heaterconfig.push_back(ss.str());
            } else if (tokens[2] == "PID") {
                // PID
                readonly = true;
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "P";
                heaterconfig.push_back(ss.str());
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "I";
                heaterconfig.push_back(ss.str());
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "D";
                heaterconfig.push_back(ss.str());
            } else if (tokens[2] == "ILIM") {
                // ILIM
                readonly = true;
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "IL";
                heaterconfig.push_back(ss.str());
            } else if (tokens[2] == "INPUT") {
                // INPUT
                readonly = true;
                ss.str("");
                ss << "MOD" << module << "/HEATER" << heaterid << "SENSOR";
                heaterconfig.push_back(ss.str());
            } else {
                // <target>
                ...
                try {
                    ...
                    if (target < this->heater_target_min || target > this->heater_target_max) {
                        message.str("");
                        message << "requested heater target " << target << " outside range {"
                    ...
                    message.str("");
                    message << "converting heater <target>=" << tokens[2] << " to float";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "heater <target>: " << tokens[2] << " outside range of float";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            ...
            if (tokens[2] == "ON") {
                ...
                try {
                    ...
                    message.str("");
                    message << "converting heater <target> " << tokens[3] << " to float";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "heater <target>: " << tokens[3] << " outside range of float";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                // ...if target is OK then push into the config, value vectors
                ...
                if (tokens[3] == "ON" || tokens[3] == "OFF") {
                    ...
                    try {
                        ...
                        if (ramprate < 1 || ramprate > 32767) {
                            message.str("");
                            message << "heater ramprate " << ramprate << " outside range {1:32767}";
                            this->camera.log_error(function, message.str());
                            return ERROR;
                        }
                    } catch (std::invalid_argument &) {
                        message.str("");
                        message << "converting RAMP <ramprate> " << tokens[3] << " to integer";
                        this->camera.log_error(function, message.str());
                        return ERROR;
                    } catch (std::out_of_range &) {
                        message.str("");
                        message << "RAMP <ramprate>: " << tokens[3] << " outside range of integer";
                        this->camera.log_error(function, message.str());
                        return ERROR;
                    }
                    ss.str("");
                ...
                try {
                    ...
                    if (il_value < 0 || il_value > 10000) {
                        message.str("");
                        message << "heater ilim " << il_value << " outside range {0:10000}";
                        this->camera.log_error(function, message.str());
                        return ERROR;
                    }
                } catch (std::invalid_argument &) {
                    message.str("");
                    message << "converting ILIM <value> " << tokens[3] << " to integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "ILIM <value>: " << tokens[3] << " outside range of integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ss.str("");
                ...
                if (tokens[3] == "A") {
                    ...
                    if (this->modtype[module - 1] != 11) {
                        message.str("");
                        message << "sensor C not supported on module " << module << ": HeaterX module required";
                        this->camera.log_error(function, message.str());
                        return ERROR;
                    }
                } else {
                    message.str("");
                    message << "invalid sensor " << tokens.at(3) << ": expected <module> A|B INPUT A|B|C";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ss.str("");
                ...
                message.str("");
                message << "expected heater <" << module << "> ON | RAMP for 3rd argument but got " << tokens[2];
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if (tokens[2] != "RAMP" && tokens[3] != "ON") {
                message.str("");
                message << "expected RAMP ON <ramprate> but got";
                for (int i = 2; i < 5; i++) message << " " << tokens[i];
                this->camera.log_error(function, message.str());
                return ERROR;
            } else {
                // got "<module> A|B RAMP ON" now check that the last (5th) token is a number
                try {
                    ...
                    message.str("");
                    message << "expected RAMP ON <ramprate> but unable to convert <ramprate>=" << tokens[4] <<
                            " to integer";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "expected RAMP ON <ramprate> but <ramprate>=" << tokens[4] <<
            ...
            if (tokens[2] != "PID") {
                message.str("");
                message << "expected PID <p> <i> <d> but got";
                for (int i = 2; i < 6; i++) message << " " << tokens[i];
                this->camera.log_error(function, message.str());
                return ERROR;
            } else {
                // got "<module> A|B PID <p> <i> <d>" now check that the last 3 tokens are numbers
                ...
                if (ret == -999) {
                    message.str("");
                    message << "comparing backplane version " << this->backplaneversion << " to " << REV_FRACTIONALPID;
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } else if (ret == -1) {
                ...
                try {
                    ...
                    if ((!fractionalpid_ok) &&
                        ...
                        // replace token with rounded integer
                        ...
                        message.str("");
                        message << "NOTICE:fractional heater PID requires backplane version " << REV_FRACTIONALPID <<
                                " or newer";
                        logwrite(function, message.str());
                        this->camera.async.enqueue(message.str());
                        message.str("");
                        message << "NOTICE:backplane version " << this->backplaneversion << " detected";
                        logwrite(function, message.str());
                        this->camera.async.enqueue(message.str());
                        message.str("");
                        message << "NOTICE:PIDs converted to: " << tokens[3] << " " << tokens[4] << " " << tokens[5];
                        this->camera.async.enqueue(message.str());
                        logwrite(function, message.str());
                    }
                    ...
                    if (pid_p < 0 || pid_p > 10000 || pid_i < 0 || pid_i > 10000 || pid_d < 0 || pid_d > 10000) {
                        message.str("");
                        message << "one or more heater PID values outside range {0:10000}";
                        this->camera.log_error(function, message.str());
                        return ERROR;
                    }
                } catch (std::invalid_argument &) {
                    message.str("");
                    message << "converting one or more heater PID values to numbers:";
                    for (int i = 3; i < 6; i++) message << " " << tokens[i];
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "heater PID exception: one or more values outside range:";
                    for (int i = 3; i < 6; i++) message << " " << tokens[i];
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            ...
            message.str("");
            message << "received " << tokens.size() << " arguments but expected 2, 3, 4, 5, or 6";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
            ...
            if (error != NO_ERROR) {
                ...
                if (key.substr(key.length() - 6) == "ENABLE" ||
                    ...
                    if (value == "0") value = "OFF";
                        ...
                        message.str("");
                        message << "bad value " << value << " from configuration. expected 0 or 1";
                        this->camera.log_error(function, message.str());
                        error = ERROR;
                    }
                    ...
                    if (value == "0") value = "A";
                        ...
                        message.str("");
                        message << "bad value " << value << " from configuration. expected 0,1,2";
                        this->camera.log_error(function, message.str());
                        error = ERROR;
                    }
                }
    ...
    long Interface::sensor(std::string args, std::string &retstring) {
        ...
        try {
            ...
            if (sensorid != "A" && sensorid != "B" && sensorid != "C") {
                message.str("");
                message << "invalid sensor " << sensorid << ": expected <module#> <A|B|C> [ current | AVG [N] ]";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        } catch (std::invalid_argument &) {
            message.str("");
            message << "parsing argument: " << args << ": expected <module#> <A|B|C> [ current | AVG [N] ]";
            this->camera.log_error(function, message.str());
            return ERROR;
        } catch (std::out_of_range &) {
            message.str("");
            message << "argument outside range in " << args << ": expected <module#> <A|B|C> [ current | AVG [N] ]";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        switch (this->modtype[module - 1]) {
            ...
            default:
                message.str("");
                message << "module " << module << " is not a heater board";
                this->camera.log_error(function, message.str());
                return ERROR;
        }
        ...
        if (tokens.size() == 2) {
            ...
            if (tokens[2] == "AVG") {
                ...
                try {
                    ...
                    message.str("");
                    message << "parsing \"" << args << "\" : expected \"AVG\" or integer for arg 3";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                } catch (std::out_of_range &) {
                    message.str("");
                    message << "parsing \"" << args << "\" : arg 3 outside integer range";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
                ...
                if (current_val < 0 || current_val > 1600000) {
                    message.str("");
                    message << "requested current " << current_val << " outside range {0:1600000}";
                    this->camera.log_error(function, message.str());
                    return ERROR;
                }
            ...
            if (tokens[2] != "AVG") {
                message.str("");
                message << "invalid syntax \"" << tokens[2] << "\". expected <module> A|B|C AVG N";
            }
            ...
            try {
                ...
                message.str("");
                message << "parsing \"" << args << "\" : expected integer for arg 4";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "parsing \"" << args << "\" : arg 4 outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            switch (filter_val) {
                case 1: sensorvalue = "0";
                    break;
                case 2: sensorvalue = "1";
                    break;
                case 4: sensorvalue = "2";
                    break;
                case 8: sensorvalue = "3";
                    break;
                case 16: sensorvalue = "4";
                    break;
                case 32: sensorvalue = "5";
                    break;
                case 64: sensorvalue = "6";
                    break;
                case 128: sensorvalue = "7";
                    break;
                case 256: sensorvalue = "8";
                    break;
                    break;
                default:
                    message.str("");
                    message << "requested average " << filter_val << " outside range {1,2,4,8,16,32,64,128,256}";
                    this->camera.log_error(function, message.str());
                    return (ERROR);
            }
        } else {
            ...
            message.str("");
            message << "received " << tokens.size() << " arguments but expected 2, 3, or 4";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    logwrite( function, message.str() );
        ...
        if (error != NO_ERROR) {
            message.str("");
            message << "reading sensor configuration " << sensorkey;
            logwrite(function, message.str());
            return error;
        }
        ...
        if (sensorkey.substr(sensorkey.length() - 6) == "FILTER") {
            ...
            try {
                ...
                message.str("");
                message << "bad value: " << value << " read back from configuration. expected integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "value: " << value << " read back from configuration outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            try {
                ...
                message.str("");
                message << "filter index " << findex << " outside range: {0:" << filter.size() - 1 << "}";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
        }
        ...
        message.str("");
        message << sensorkey << "=" << value << " (" << retstring << ")";
        logwrite(function, message.str());
    ...
    long Interface::bias(std::string args, std::string &retstring) {
        ...
        if (tokens.size() == 2) {
            ...
            message.str("");
            message << "incorrect number of arguments: " << args << ": expected module channel [voltage]";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        try {
            ...
            message.str("");
            message << "parsing bias arguments: " << args << ": expected <module> <channel> [ voltage ]";
            this->camera.log_error(function, message.str());
            return ERROR;
        } catch (std::out_of_range &) {
            message.str("");
            message << "argument range: " << args << ": expected <module> <channel> [ voltage ]";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if ((module < 0) || (module > nmods)) {
            message.str("");
            message << "module " << module << ": outside range {0:" << nmods << "}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if ((channel < 1) || (channel > 30)) {
            message.str("");
            message << "bias channel " << module << ": outside range {1:30}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if ((voltage < vmin) || (voltage > vmax)) {
            message.str("");
            message << "bias voltage " << voltage << ": outside range {" << vmin << ":" << vmax << "}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
    ...
    long Interface::inreg(std::string args) {
        ...
        try {
            ...
            message.str("");
            message << "unable to convert one of \"" << args << "\" to integer";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        catch (std::out_of_range &) {
            message.str("");
            message << "one of \"" << args << "\" is outside integer range";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        switch (this->modtype[module - 1]) {
            case 0:
                message.str("");
                message << "requested module " << module << " not installed";
                this->camera.log_error(function, message.str());
                return ERROR;
            case 3: // LVBias
            case 5: // Heater
            ...
            default:
                message.str("");
                message << "requested module " << module << " does not contain a VCPU";
                this->camera.log_error(function, message.str());
                return ERROR;
        }
        ...
        if (reg < 0 || reg > 15) {
            message.str("");
            message << "requested register " << reg << " outside range {0:15}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if (value < 0 || value > 65535) {
            message.str("");
            message << "requested value " << value << " outside range {0:65535}";
            this->camera.log_error(function, message.str());
            return ERROR;
        }
        ...
        if (error != NO_ERROR) {
            message.str("");
            message << "configuration " << inreg_key.str() << "=" << value;
            logwrite(function, message.str());
            return ERROR;
        } else {
            std::stringstream applystr;
    ...
    long Interface::test(std::string args, std::string &retstring) {
        ...
        if (testname == "ampinfo") {
            ...
            message.str("");
            message << "[ampinfo] observing mode=" << mode;
            logwrite(function, message.str());
            message.str("");
            message << "[ampinfo] FRAMEMODE=" << this->modemap[mode].geometry.framemode;
            logwrite(function, message.str());
            message.str("");
            message << "[ampinfo] LINECOUNT=" << this->modemap[mode].geometry.linecount << " PIXELCOUNT=" << this->
                    modemap[mode].geometry.pixelcount;
            logwrite(function, message.str());
            message.str("");
            message << "[ampinfo] num_taps=" << this->modemap[mode].tapinfo.num_taps;
            logwrite(function, message.str());
            message.str("");
            message << "[ampinfo] hori_amps=" << this->modemap[mode].geometry.amps[0] << " vert_amps=" << this->modemap[
                mode].geometry.amps[1];
            logwrite(function, message.str());

            message.str("");
            message << "[ampinfo] gains =";
            ...
            for (int y = 0; y < vamps; y++) {
                for (int x = 0; x < hamps; x++) {
                    ...
                    message.str("");
                    message << "[ampinfo] x0=" << x0 << " x1=" << x1 << " y0=" << y0 << " y1=" << y1
                            << " | amp section (xrange, yrange) " << (x0 * cols + 1) << ":" << (x1) * cols << ", " << (
                                y0 * rows + 1) << ":" << (y1) * rows;
                    logwrite(function, message.str());
                }
            }
            ...
            if (tokens.size() == 1) {
                message.str("");
                message << "archon_busy=" << this->archon_busy;
                logwrite(function, message.str());
                error = NO_ERROR;
            } else if (tokens.size() == 2) {
                try {
                ...
                message.str("");
                message << "expected one argument, yes or no";
                this->camera.log_error(function, message.str());
                error = ERROR;
            }
            ...
            if (tokens.size() > 1) {
                ...
                // if no string passed then queue a simple test message
            ...
            for (auto &mode_it: this->modemap) {
                ...
                message.str("");
                message << "found mode section " << mode;
                logwrite(function, message.str());
                for (auto param_it = this->modemap[mode].parammap.begin();
                     param_it != this->modemap[mode].parammap.end(); ++param_it) {
                    message.str("");
                    message << "MODE_" << mode << ": " << param_it->first << "=" << param_it->second.value;
                    logwrite(function, message.str());
                }
            }
            ...
            for (auto &param_it: this->parammap) {
                keycount++;
                message.str("");
                message << param_it.first << "=" << param_it.second.value;
                logwrite(function, message.str());
            ...
            for (auto &mode_it: this->modemap) {
                ...
                message << "found mode section " << mode;
                logwrite(function, message.str());
                for (auto config_it = this->modemap[mode].configmap.begin();
                     config_it != this->modemap[mode].configmap.end(); ++config_it) {
                    message.str("");
                    message << "MODE_" << mode << ": " << config_it->first << "=" << config_it->second.value;
                    logwrite(function, message.str());
                }
            }
            ...
            message.str("");
            message << "found " << keycount << " configmap entries";
            logwrite(function, message.str());
            ...
            if (tokens.size() > 2) {
                try {
                    if (tokens.at(2) == "rw") rw = true;
                    else rw = false;
                    if (tokens.at(2) == "ro") ro = true;
                    else ro = false;
                } catch (std::out_of_range &) {
            ...
            try {
                ...
                message.str("");
                message << "unable to convert sequences: " << nseqstr << " to integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "sequences " << nseqstr << " outside integer range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            if (error == NO_ERROR) {
                ...
                if (rw) {
                    ...
                    error = this->camera.get_fitsname(this->camera_info.fits_name);
                    // assemble the FITS filename if rw selected
                    if (error != NO_ERROR) {
                        logwrite(function, "ERROR: couldn't validate fits filename");
                        return error;
                    }
            ...
            while (nseq-- > 0) {
                ...
                if (error == NO_ERROR) error = this->wait_for_readout(); // wait for the readout into frame buffer,
                if (error == NO_ERROR && ro) error = this->read_frame(Camera::FRAME_IMAGE);
                // read image frame directly with no write
            ...
            message.str("");
            message << "frames read = " << frames_read;
            logwrite(function, message.str());
            // end if (testname==bw)
            ...
            try {
                ...
                message.str("");
                message << "unable to convert one or more args to an integer";
                this->camera.log_error(function, message.str());
                return ERROR;
            } catch (std::out_of_range &) {
                message.str("");
                message << "nseq, sleepus tokens outside range";
                this->camera.log_error(function, message.str());
                return ERROR;
            }
            ...
            while (error == NO_ERROR && nseq-- > 0) {
                ...
                //
                message.str("");
                message << "TEST_TIMER: " << nseqsave - nseq << ", " << delta_archon << ", " << delta_system;
                this->camera.async.enqueue(message.str());

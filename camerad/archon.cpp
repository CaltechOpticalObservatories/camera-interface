...
long Interface::archon_cmd(std::string cmd, std::string &reply) {
    ...
    const int sleep_interval_us = 100;
    char check[4];
    char buffer[4096];
    int retval;
    int error = NO_ERROR;
...
*********/
  ...
  long Interface::fetch(uint64_t bufaddr, uint32_t bufblocks) {
    ...
    if (!this->is_autofetch) {
      if(this->pause_log == false) {
        logwrite(function, "Fetching frame buffer from Archon controller...");
      }
...
//  this->lastframe = this->frame.bufframen[this->frame.index];     // save the last frame number acquired (wait_for_readout will need this)
  ...
  /**
    ...
    */
    long Interface::hsetup() {
        ...
        error = this->inreg("10 1 16402");      // 0100 000000010010 //WinOutput = 0100 000000011010
        std::this_thread::sleep_for(std::chrono::seconds(1));
     ...
     long Interface::set_freerun(std::string set_state){
      ...
      if ( !set_state.empty() ) {
          try {
              ...
              std::string freerun_param = "freerun";
              std::string abort_param = "Abort";
              long new_state, abort;
              if (set_state == "TRUE" || set_state == "1") {
                //set expose and start on the archon or acf to 1
                new_state = 1;
                abort = 0;
                error = this->set_parameter( start_param, new_state);
                if ( error == NO_ERROR ) error = this->set_parameter( abort_param, abort); // issue an abort to stop any exposures in progress
                if ( error == NO_ERROR ) error = this->set_parameter( freerun_param, new_state);
                //log
                ...
                new_state = 0;
                abort = 1;
                error = this->set_parameter( abort_param, abort); // issue an abort to stop any exposures in progress
                if ( error == NO_ERROR ) error = this->set_parameter( freerun_param, new_state);
  ...
  /**************** Archon::Interface::hexpose ******************************/
    ...
    while (remaining-- > 0 && this->lastframe < finalframe) {
        ...
        if (is_zmq) {
          int16_t *cbuf16s;
          cbuf16s = reinterpret_cast<int16_t *>(this->image_data);                        // cast to 16b signed int
          std::stringstream ss;
    ...
    long Interface::fexpose(int nseq_in) {
      ...
      //logwrite(function, "start frame: " + std::to_string(startframe) + ", last frame: " + std::to_string(this->lastframe));
      //int sample_size = 100000;
      //double results[sample_size][3] = {};
      ...
      while (this->abort_fexpose == false && this->is_freerun == true && this->abort == false) {
        ...
        while (true) {
          ...
          std::this_thread::sleep_for(std::chrono::microseconds(100)); // sleep briefly to avoid busy waiting
        }
        //if ( error != NO_ERROR ) {
        //  logwrite( function, "ERROR: waiting for readout" );
        //  return error;
        ...
        //if (is_timing_test) {
        //  results[loopIter][2] = 0.0; // placeholder for shared memory write timing, to be filled in when that is implemented
        //}
        //if (is_timing_test && (loopIter == (sample_size - 1))) {
        //  this->writeSignalCSV(results, save_filename, sample_size);
        //  loopIter++;
        //}
      }
      logwrite(function, "fexpose loop end.\n");
      // --- Sequence Complete ---
      ...
      logwrite(function, "start frame: " + std::to_string(startframe) + ", last frame: " + std::to_string(lastframe));
    ...
    long Interface::fabort(std::string &retstring) {
      ...
      //logwrite(function, "Frames Read: " + std::to_string(this->fread));
      //std::cout << std::fixed << std::setprecision(4) << "Median Loop Time = " << fetch_stats.median() << " us"
      //          << " | Mean Loop Time = " << fetch_stats.mean() << " us"
      //          << " | Jitter = " << fetch_stats.jitter() << " us" << std::endl;
    ...
    /**************** Archon::Interface::fabort_print *********************************/
    /**
     * @fn     fabort_print
     * @brief  prints and logs small amount of data at the end of abort
     * @return ERROR or NO_ERROR
     */
    long Interface::fabort_print() {
      //Iniciate variables and messages for logs
      const std::string function = "Archon::Interface::fabort_print";
      std::stringstream message;
      long error = NO_ERROR;

      //Print what fabort cannot
      logwrite(function, "Frames Read: " + std::to_string(this->fread));
      message.str("");
      message << "READOUT SEQUENCE " << (error == NO_ERROR ? "COMPLETE" : "ERROR")
              << " (" << this->fread << " frames read)";
 ...
 * Function to save an array into a CSV file
 ======*/
 /** 
 void Interface::writeSignalCSV(double array[][3], const std::string& filename, int Nrows) 
 {
...
void Interface::writeSignalCSV(double array[][3], const std::string& filename, int Nrows) 
{
    ...
    // Save default precision to reset later if needed
    std::streamsize defaultPrec = file.precision();

    for (int row = 0; row < Nrows; row++) 
    {
        // We only have 3 columns now: 0, 1, and 2
        for (int col = 0; col < 3; col++) 
        {
            // Since these are all timing points (likely Unix timestamps or nanoseconds),
            // it is best to keep high precision for all of them.
            file.precision(20); 
            file << array[row][col];

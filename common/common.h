...
namespace Common {
    ...
    class PubSub {
        public:
            ...
            PubSub(zmqpp::context& context, Mode mode)
                : socket(context, mode == Mode::PUB ? zmqpp::socket_type::publish
            ...
            void send(const std::string& topic, const std::string& payload) {
                if (mode != Mode::PUB) return;
                zmqpp::message msg;
                msg << topic << payload;
                socket.send(msg);
            }

            void publish(const nlohmann::json& json_message) {
                //TODO: send json message via pub sub
            }
        
            bool receive(std::string& topic, std::string& payload) {
                if (mode != Mode::SUB) return false;
                zmqpp::message msg;
                if (!socket.receive(msg, /*dont_wait=*/true)) return false;
                msg >> topic >> payload;
                return true;
            }
    ...
    class PubSubHandler {
    public:
        static long init_pubsub(zmqpp::context& context, T& iface,
                                const std::initializer_list<std::string>& topics) {
            iface.publisher = std::make_unique<PubSub>(context, PubSub::Mode::PUB);
            iface.subscriber = std::make_unique<PubSub>(context, PubSub::Mode::SUB);

            iface.publisher_address = "tcp://*:5556";
            iface.subscriber_address = "tcp://localhost:5556";

            iface.publisher->bind(iface.publisher_address);
            iface.subscriber->connect(iface.subscriber_address);

            iface.subscriber_topics.assign(topics.begin(), topics.end());
            return 0;
        }

        static void start_subscriber_thread(T& iface) {
            if (iface.is_subscriber_thread_running)
                return;

            iface.should_subscriber_thread_run = true;
            iface.is_subscriber_thread_running = true;

            std::thread([&iface]() {
                std::string topic, payload;
                while (iface.should_subscriber_thread_run) {
                    if (iface.subscriber->receive(topic, payload)) {
                        auto it = iface.topic_handlers.find(topic);
                        if (it != iface.topic_handlers.end()) {
                            try {
                ...
                iface.is_subscriber_thread_running = false;
            }).detach();
        }

        static void stop_subscriber_thread(T& iface) {
            iface.should_subscriber_thread_run = false;
        }
    };

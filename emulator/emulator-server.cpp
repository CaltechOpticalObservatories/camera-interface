...
void doit(Network::TcpSocket sock) {
    ...
    while (connection_open) {
        ...
        if (cmd.compare("SYSTEM") == 0) {
            ...
            retstream << (ret == ERROR ? "?" : "<") << ref << retstring;
        } else if (cmd.compare("TIMER") == 0) {
            std::string retstring;
            ret = server.timer_report(retstring);
            retstream << (ret == ERROR ? "?" : "<") << ref << "TIMER=" << retstring;
        } else if (cmd.compare("FRAME") == 0) {
            std::string retstring;
            ret = server.frame_report(retstring);
            retstream << (ret == ERROR ? "?" : "<") << ref << retstring;
        } else if (cmd.compare("FETCHLOG") == 0) {
            retstream << "<" << ref << "(null)";
        } else if (cmd.compare(0, 4, "LOCK") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare(0, 5, "FETCH") == 0) {
            ret = server.fetch_data(ref, cmd, sock);
            retstream.str(""); // FETCH returns no message
        } else if (cmd.compare(0, 7, "WCONFIG") == 0) {
            ret = server.wconfig(cmd);
            retstream << (ret == ERROR ? "?" : "<") << ref;
        } else if (cmd.compare(0, 7, "RCONFIG") == 0) {
            std::string retstring;
            ret = server.rconfig(cmd, retstring);
            retstream << (ret == ERROR ? "?" : "<") << ref << retstring;
        } else if (cmd.compare("CLEARCONFIG") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("APPLYALL") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("APPLYSYSTEM") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("POWERON") == 0) {
            server.poweron = true;
            retstream << "<" << ref;
        } else if (cmd.compare("POWEROFF") == 0) {
            server.poweron = false;
            retstream << "<" << ref;
        } else if (cmd.compare("LOADTIMING") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("LOADPARAMS") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare(0, 9, "LOADPARAM") == 0) {
            ret = server.write_parameter(cmd.substr(14));
            retstream << (ret == ERROR ? "?" : "<") << ref;
        } else if (cmd.compare(0, 9, "PREPPARAM") == 0) {
            //                  server.write_parameter( cmd.substr(14) );
            retstream << "<" << ref;
        } else if (cmd.compare(0, 13, "FASTLOADPARAM") == 0) {
            ret = server.write_parameter(cmd.substr(14));
            retstream << (ret == ERROR ? "?" : "<") << ref;
        } else if (cmd.compare(0, 13, "FASTPREPPARAM") == 0) {
            //                  server.write_parameter( cmd.substr(14) );
            retstream << "<" << ref;
        } else if (cmd.compare("RESETTIMING") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("HOLDTIMING") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("RELEASETIMING") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare(0, 8, "APPLYMOD") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare(0, 8, "APPLYDIO") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("APPLYCDS") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("POLLOFF") == 0) {
            retstream << "<" << ref;
        } else if (cmd.compare("POLLON") == 0) {
            retstream << "<" << ref;
        } else {
            // Archon simply ignores things it doesn't understand
        }
#endif

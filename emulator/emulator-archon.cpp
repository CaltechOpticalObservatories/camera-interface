...
namespace Archon {
    ...
    long Interface::configure_controller() {
        ...
        for (int entry = 0; entry < this->config.n_entries; entry++) {
            try {
                ...
                std::cerr << function << "ERROR: invalid argument parsing entry " << entry << " of " << this->config.
                        n_entries << "\n";
                return (ERROR);
            }
            catch (std::out_of_range &) {
                std::cerr << function << "ERROR: value out of range parsing entry " << entry << " of " << this->config.
                        n_entries << "\n";
                return (ERROR);
            }
            catch (...) {
    ...
    long Interface::frame_report(std::string &retstring) {
        ...
        if (this->image.activebufs == -1) {
            std::cerr << function <<
                    "ERROR: activebufs undefined. Check that an ACF was loaded and that it contains BIGBUF=x\n";
            return (ERROR);
        }
        ...
        try {
            ...
            std::cerr << function << "ERROR: buffer number " << bufn + 1 << " out of range {1:" << this->image.
                    activebufs << "}\n";
            return (ERROR);
        }
        catch (...) {
        ...
        retstring = retstring.erase(retstring.find_last_not_of(" ") + 1);
        // remove trailing space added because of for loop
    ...
    long Interface::fetch_data(std::string ref, std::string cmd, Network::TcpSocket &sock) {
        ...
        if (cmd.length() != 21) {
            // must be "FETCHxxxxxxxxyyyyyyyy", 21 chars
        ...
        std::cerr << function << "host requested " << std::dec << reqblocks << " (0x" << std::hex << reqblocks <<
                ") blocks \n";
    ...
    long Interface::rconfig(std::string buf, std::string &retstring) {
        ...
        if (buf.length() != 11) {
            // check for minimum length "RCONFIGxxxx", 11 chars
            return (ERROR);
        }
    ...
    long Interface::write_parameter(std::string buf) {
        ...
        try {
            ...
            if (key == "exptime") {
                ...
                std::cerr << function << "exptime = " << this->image.exptime << (this->image.exposure_factor == 1
                    ? " sec"
                    : " msec") << "\n";
            }
    ...
    void Interface::dothread_expose(frame_data_t &frame, image_t &image, int numexpose) {
        ...
        for (int num = 0; num < numexpose; num++) {
            ...
            try {
                ...
                for (frame.buflines.at(frame.index) = 0; frame.buflines.at(frame.index) < image.linecount; frame.
                     buflines.at(frame.index)++) {
                    for (frame.bufpixels.at(frame.index) = 0; frame.bufpixels.at(frame.index) < image.pixelcount; frame.
                         bufpixels.at(frame.index)++) {
